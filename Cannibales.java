...
public class Cannibales extends Neuneu{
	...
	public Cannibales(Cannibales l, Loft lf){
		super(l, lf);
	}
...
public void manger(Aliment mange, int quantite, Loft plateau){
		...
		if (plateau.plateauCase.get(this.getPosition()).contenuNeuneu.size() >1){
			int k=0;
			
			for (Neuneu neuneu : plateau.plateauCase.get(this.getPosition()).contenuNeuneu ){
					while (k==0){
						if (neuneu.getId_joueur() != this.getId_joueur()){
														// position ddu neuneu dans la liste d'aliment que contient la case
							// augmenter energie du cannibale
							this.energie=this.energie + neuneu.getEnergie();
							
							// supprimer le neuneu mange de la partie
							plateau.plateauCase.get(this.position).contenuNeuneu.remove(neuneu);
							plateau.population.remove(neuneu);
							k+=1;					
...
public void seDeplacer(int position, Loft plateau){
	...
	public void seReproduire(Loft plateau){
		...
		if ((plateau.plateauCase.get(position).contenuNeuneu.size()>1)&&(this.energie>10)&&(plateau.plateauCase.get(position).contenuNeuneu.size()<3)){
			...
			plateau.plateauCase.get(this.position).contenuNeuneu.add(new Cannibales(this,plateau));
