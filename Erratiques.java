...
public class Erratiques extends Neuneu{
	...
	// CONSTRUCTEUR
		...
		public Erratiques(Erratiques l, Loft lf){
			super(l, lf);
		}
		...
		public void boire(Aliment boire, int quantite, Loft plateau){
			...
			public void seDeplacer(int position, Loft plateau){
				...
				int m = -2+ rd.nextInt(5);
				
				if ( !((this.getEnergie()==0) || (m==3) || (m==0) || (this.getPosition()+ m < 0) || (this.getPosition()+ m > 11) )){
			...
			public void seReproduire(Loft plateau){
				...
				if ((plateau.plateauCase.get(position).contenuNeuneu.size()>1)&&(this.energie>10)&&(plateau.plateauCase.get(position).contenuNeuneu.size()<3)){
					...
					plateau.plateauCase.get(this.position).contenuNeuneu.add(new Erratiques(this, plateau));
